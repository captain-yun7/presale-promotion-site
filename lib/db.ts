...
export async function submitInquiry(data: Inquiry) {
  const rows = await sql`
    INSERT INTO inquiries (name, phone, unit_type, message)
    VALUES (${data.name}, ${data.phone}, ${data.unit_type || null}, ${data.message || null})
    RETURNING *
  `;
  return rows;
}
...
export async function submitConsultation(data: Consultation) {
  const rows = await sql`
    INSERT INTO consultations (name, phone, message, source, project)
    VALUES (
      ${data.name},
      ${data.phone},
      ${data.message || null},
